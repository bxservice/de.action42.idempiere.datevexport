...
public class OBE_Verwaltungsdatei {
	...
	public OBE_Verwaltungsdatei(final File targetDirectory)
	 ...
	 */
	public void writeKennsatz(final OBE_Datentraegerkennsatz kennsatz)
			throws DatevException {
		...
		try {
			...
			for (int i = writer.size(); i < 64; i++) {
				writer.write(OBE_Datentraegerkennsatz.FILLER);
			}
	 ...
	 */
	public void writeVerwaltunssatz(final OBE_Verwaltungssatz verwaltungssatz)
			throws DatevException {
		...
		try {
			...
			if (verwaltungssatz.getType() == OBE_Verwaltungssatz.Type.STAMMDATEN) {
				for (int i = 0; i < 16; i++) {
			...
			for (int i = 0; i < 8; i++) {
				writer
						.write(OBE_Verwaltungssatz.LAUFENDE_ABSTIMMSUMME_RESERVIERT);
			}
